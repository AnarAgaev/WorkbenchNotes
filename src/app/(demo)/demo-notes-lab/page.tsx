...
import {getNoteById, getNotes} from '@/demo/notesStore'
		...
		<Container className="py-10">
			<header className="flex flex-wrap items-start justify-between gap-4">
				<div className="space-y-2">
					...
					<p className="text-sm muted">
						Выбор заметки задаётся через{' '}
						<code className="font-mono">searchParams.note</code>. Невалидный
						ввод не приводит к 404.
					</p>
				</div>
			...
			{(isInvalidNoteParam || isMissingNote) && (
				<div className="mt-6 app-card">
					...
					{isMissingNote && (
						<p className="mt-1 text-sm muted">
							Заметка с id <code className="font-mono">{noteId}</code> не
							найдена в хранилище.
						</p>
					)}
				</div>
					...
					{selectedNote ? (
						...
						<>
							...
							<div className="mt-3 text-sm muted">
								Пример:{' '}
								<code className="font-mono">/demo-notes-lab?note=n1</code>
							</div>
						</>
					)}
				</div>
