...
import {type FormErrors, validateTitle} from '@/demo/formContract'
...
export async function submitTitleServer(
	rawTitle: string,
	...
	if (title.toLowerCase() === 'crash') {
		return {
			ok: false,
			errors: {formError: 'Сервер временно недоступен. Повторите позже.'},
		}
	}
